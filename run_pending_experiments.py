...
def print_exp_to_run(d, n_runs):
    print('********************************************************')
    print('Setting: {:s} {:s} nkw={:d}'.format(d['dataset'], str(d['gen_params']), d['nkw']))
    print('Query: {:s} {:s}'.format(d['query_name'], str(d['query_params'])))
    print('Def: {:s} {:s}'.format(d['def_name'], str(d['def_params'])))
    print('Att: {:s} {:s}'.format(d['att_name'], str(d['att_params'])))
...
if __name__ == "__main__":
    ...
    for file in sorted(os.scandir(EXPERIMENTS_PATH), key=lambda e: e.name):
        if file.name.startswith('todo_') and file.name.endswith('.pkl'):
            ...
            os.remove(file)
            print("Read and deleted {:s}".format(file.name), flush=True)
            ...
            EXTENSION = '{:s}_{:d}_{:s}_{:s}'.format(parameter_dict['dataset'], parameter_dict['nkw'],
            ...
            print("**** Done experiments, elapsed time {:.0f} (total {:.0f})".format(time.time() - time_init_this_exp,
