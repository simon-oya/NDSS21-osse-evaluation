...
exp_params_column = ['dataset', 'nkw', 'gen_params', 'query_name', 'query_params',
                     'def_name', 'def_params', 'att_name', 'att_params',
...
class ManagerDf:
    ...
    def print_all(self):
        ...
        results_table = pd.DataFrame(np.array(results_table_rows), columns=['acc', 'time', 'runs'])

        print(pd.concat([self.experiments[['nkw']], self.experiments['gen_params'].apply(get_odd),
                         self.experiments[['query_name']], self.experiments['query_params'].apply(get_odd),
                         self.experiments[['def_name']], self.experiments['def_params'].apply(get_odd),
                         self.experiments[['att_name']], self.experiments['att_params'].apply(get_odd),
    ...
    def print_results_given_indices(self, indices):
        ...
        #      len(self.results[pointer].index)]
        #     for pointer in smaller_df['res_pointer']]
        results_df = pd.DataFrame(np.array(results_table_rows), index=indices, columns=['acc', 'time', 'runs'])
        # concat_df = pd.concat([smaller_df, results_df], axis=1)
        print(pd.concat([smaller_df[['nkw']], smaller_df['gen_params'].apply(get_odd),
                         smaller_df[['query_name']], smaller_df['query_params'].apply(get_odd),
                         smaller_df[['def_name']], smaller_df['def_params'].apply(get_odd),
                         smaller_df[['att_name']], smaller_df['att_params'].apply(get_odd),
